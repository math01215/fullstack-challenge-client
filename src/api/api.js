...
async function getPageComments(pageId) {
  ...
  return pageComments.data.comments
}

async function postPageComment(comment) {
  const newComments = await api.post(`pages/${comment.page}/comments/`, comment)
  return newComments.data.comments
}
